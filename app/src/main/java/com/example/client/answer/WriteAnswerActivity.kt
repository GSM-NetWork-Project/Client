...
class WriteAnswerActivity : AppCompatActivity() {
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        btn_write_answer.setOnClickListener {
            when {
                write_answer_text.text.toString().length > 250 -> {
                    val dialog =
                ...
                write_answer_text.text.toString().isEmpty() -> {
                    val dialog =
                        SweetAlertDialog(this@WriteAnswerActivity, SweetAlertDialog.ERROR_TYPE)
                    ...
                    checkSwearing(write_answer_text.text.toString())
                }
            }
        }
                ...
                if (response.isSuccessful){
                    if(response.body()!!.status == 200){
                        ...
                        if(intent.getBooleanExtra("is_edit_answer", false)){
                            RetrofitHelper().getModifyAPI().modifyAnswer(intent.getIntExtra("question_id",0), getID(), text).enqueue(object : Callback<Status>{
                            ...
                            RetrofitHelper().getAddAPI().addAnswer(intent.getIntExtra("question_id",0), getID(), text).enqueue(object : Callback<Status>{
                                override fun onResponse(call: Call<Status>, response: Response<Status>) {
                                    if(response.isSuccessful){
                                        if(response.body()!!.status == 200){
                                            val dialog =
